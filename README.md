...
Correlating with this, the UFW firewall running on the Ubuntu server has the following rules table:
...
These two systems in coordination work to serve the web application and for development access outside of the virtual machine.
...
[This guide](https://www.digitalocean.com/community/tutorials/how-to-configure-apache-http-with-mpm-event-and-php-fpm-on-ubuntu-18-04) was followed to install necessary dependencies for running Symfony through Apache. Using [the Symfony documentation](symfony.com/doc/current/setup/web_server_configuration.html) as a guide, the /etc/apache2/sites-enabled/000-default.conf file was changed to the code below, to serve the Symfony application from it's directory:
...
<VirtualHost *:80>
        ...
        DocumentRoot /home/senior-project/patent-management-system/public

        <Directory /home/senior-project/patent-management-system/public>
                AllowOverride None
                Require all granted
...
### MariaDB
Installation and configuration was simple for me following [the guide here](https://www.digitalocean.com/community/tutorials/how-to-install-mariadb-on-ubuntu-20-04). The biggest change needed was that when connecting to the MariaDB server remotely, the host address was not seen by MariaDB in the guest as 'localhost', but as '_gateway'. This was fixed by creating a new account with `GRANT ALL ON *.* TO 'admin'@'gateway' IDENFITIED BY (password)`, which was accessible over port 3306 to our host machine under the hostname 'localhost'. 

In addition to this, the 'bind-address' for MariaDB, the address range in which the MariaDB database should be accessible from, needed to be changed for development purposes. Under /etc/mysql/mariadb.cnf, the following lines were added at the end of the file for allow for remote access:
